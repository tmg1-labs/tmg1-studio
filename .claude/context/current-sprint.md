...
- **「新規作成／開く でファイル選択後に何も起きない」問題の原因究明と修正**（2026-08-13、
  **v0.1.2 として公開済み。本件は完了**）。
  ...
  - **実因（probe を失敗させていた要因）は未特定**。検証時点では ffmpeg/ffprobe/tmg1 とも正常に
    解決でき、ユーザー操作も成功したため再現せず。環境側の一過性要因（8/10 に winget が ffmpeg を
    9.0 へ上げて `WinGet\Links` のシンボリックリンクを張り替え／8/12 に WebView2 が 151.0.4129.78 へ
    更新）が時系列上は近いが、**断定はしていない**。
...
- **マニュアル（GitHub Pages）に今回の UI 変更が未反映**（2026-08-13 にユーザー判断で見送り）。
  ...
  追記するなら、スクショの差し替えも要る。
- **「ファイル選択後に無反応」の実因は未特定のまま**。再発したら、今度はアプリがエラー本文を
  画面に出すので、そのメッセージから切り分ける。
- 未検討: 「名前を付けて保存」（別名保存）。
