...
最終更新: 2026-08-13（「ファイル選択後に無反応」の原因究明＋ステータスバー常時表示／ツール
プリフライト実装 → **v0.1.2 としてリリース公開済み**）
...
- **「新規作成／開く でファイル選択後に何も起きない」問題の原因究明と修正**（2026-08-13、
  **v0.1.2 として公開済み。本件は完了**）。
  ...
  - **実因＝起動元プロセスの環境の継承（2026-08-13 夜に特定）**。当初は再現せず未特定としていたが、
    v0.1.2 リリース後にユーザーが**同じ警告（ffmpeg/ffprobe/tmg1 が見つからない）を再現**したため、
    実行中プロセスの PATH を PEB から実測して確定した。
    - `explorer.exe`(PID 1956) が **12:51 にマシン PATH だけの環境で再起動**されており、そこから
      起動した Chrome → ダウンロードしたインストーラ → **インストーラ完了画面から起動されたアプリ**、
      と壊れた環境が伝播していた。ユーザー PATH 側にある `WinGet\Links`（ffmpeg/ffprobe）と
      `~/.cargo/bin`（tmg1）が丸ごと欠落するため 3 つとも解決できない。
      実測: 正常起動 3410字/84エントリ ↔ 壊れた起動 1938字/52エントリ。
    - **「スタートメニューから起動すれば直る」は誤り**（一度そう誤診した）。スタートメニューも
      Explorer の子なので同じ環境を継承する。詳細と診断手順は known-issues.md。
    - 対処はユーザー選択で**サインアウト→サインイン**（根本対処）。別解として設定の
      「実行ファイルのパス」に絶対パスを入れる方法もある（起動元非依存）。
    - Explorer がなぜ壊れた環境で再起動されたかは**不明**のまま（同時刻に winget のログはあるが断定不可）。
...
- **マニュアル（GitHub Pages）に今回の UI 変更が未反映**（2026-08-13 にユーザー判断で見送り）。
  ...
  追記するなら、スクショの差し替えも要る。
- **実因は特定済み**（起動元プロセスの環境継承。上記参照）。ユーザーはサインアウト→サインインで
  対処する方針。**サインイン後に警告バーが消えるかの確認は未実施**（次セッションで確認する）。
  再発する場合は設定の「実行ファイルのパス」に絶対パスを入れる方へ切り替える。
- 未検討: 「名前を付けて保存」（別名保存）。
