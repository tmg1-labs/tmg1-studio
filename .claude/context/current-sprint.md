...
- **「新規作成／開く でファイル選択後に何も起きない」問題の原因究明と修正**（2026-08-13、
  **実装・実機検証済み・未コミット**）。
  ...
  - 残: 配布版インストーラの再ビルド（`npm run tauri build`）と commit。
...
## 次にやること
- ステータスバー常時表示＋ツールプリフライト（2026-08-13）を commit する。**リリースビルドは
  ローカル版 `0.1.0` になる**点に注意（配布済みインストーラは CI が `sync-version.mjs` でタグ値に
  揃えた `0.1.1`）。配布版として出すならタグを切って release.yml に流すのが筋。
