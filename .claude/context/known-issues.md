...
### 合計 PATH が 4094 字を超えると Explorer がユーザー PATH を丸ごと捨てる（2026-08-14 実測で確定）
> **これが「ffmpeg/ffprobe/tmg1 が見つからない」「ファイル選択後に無反応」事件の真因**。
> 2026-08-13 に「Explorer がマシン PATH だけの環境で**再起動**された／サインアウト→サインインで
> 根治」と結論づけたが**誤り**だった（PC を再起動しても再現して否定された）。

- **症状**: **ターミナルでは `ffmpeg`/`ffprobe`/`tmg1` が普通に動くのに、スタートメニューから
  起動したアプリだけ「見つかりません」**。再起動しても直らない。
- **原因（機構）**: Explorer は `WM_SETTINGCHANGE("Environment")` を受けると**自分の環境を
  レジストリから再生成**する。このときの結果は `マシン PATH + ";" + ユーザー PATH`（重複除去なし）だが、
  **合計が 4095 字以上になるとユーザー PATH が丸ごと捨てられ、マシン PATH のみに転落する**
  （エラーも警告も出ない）。子プロセスは親の環境を丸ごと継承するので、以降 Explorer から起動した
  ものが全滅する。`ffmpeg`/`ffprobe`（`%LOCALAPPDATA%\Microsoft\WinGet\Links`）も
  `tmg1`（`~/.cargo/bin`）も**ユーザー PATH 側**にあるため 3 つとも解決できなくなる。
- **閾値（1 字刻みで実測）**: 合計 **4094 字までは通り、4095 字で脱落**。4096 字バッファの終端 2 字
  予約と整合する。**「2048 字」説は否定済み**（ユーザー PATH 2205 字は通過した）。
  2026-08-14 の実環境: マシン 1887 + 1 + ユーザー 2262 = **4150 字**（56 字オーバー）で常に脱落していた。
- **無実だと確認したもの**（ここを疑って時間を溶かさない）:
  - レジストリの PATH（HKCU/HKLM とも正常・型も `ExpandString`・実体も存在）
  - `CreateEnvironmentBlock`（同じレジストリから 3410 字/84 エントリを正しく生成できる。
    サービスや DCOM 経由で起動されたプロセスはこちらなので**健全**）
  - ログオン処理（boot type 0x0 の通常ブート、プロファイルも正常ロード）
  - アプリ側の実装（`ExePaths::load` / `check_tools` は正しく動いていた）
- **決め手になった 2 つの実験**:
  1. **時刻境界**: 同一の親 explorer(同一 PID)の子なのに、ある時刻より前の子は 4150 字、後の子は
     1887 字。**Explorer 自身の環境が途中で差し替わった**ことがこれで確定する（＝再起動ではない）。
  2. **対照実験**: PATH に触れず、小さなダミー変数（例 `TMG1_ENVTEST`）を `HKCU\Environment` に
     一時追加して `WM_SETTINGCHANGE` をブロードキャストする。**ダミーは Explorer の環境に現れるのに
     PATH だけ変わらない**なら「再生成はしているがユーザー PATH だけ捨てている」と断定できる。
     これをやらないと「再生成が壊した」のか「そもそも再生成していない」のか区別できない。
- **対処**:
  1. **合計 PATH を 4094 字以内にする（根治）**。まず**存在しないディレクトリ**と**完全重複**を削る。
     2026-08-14 の実績: ユーザー PATH から実体の無い 14 件＋重複 2 件を削除して 2261 → 1516 字、
     合計 4150 → **3404 字**。削除後に `WM_SETTINGCHANGE` をブロードキャストすれば
     **サインアウト不要**でその場の Explorer が healthy になる（3404 字/87 エントリを確認）。
     アプリは古い環境を持ったままなので**起動し直すこと**。
  2. **設定メニューの「実行ファイルのパス」に絶対パスを入れる**（Studio だけを起動元非依存にする保険）。
     winget のものは `WinGet\Links\ffmpeg.exe` を指すこと。実体の
     `WinGet\Packages\...\ffmpeg-9.0-full_build\bin\` を直指しするとアップグレードで壊れる。
- **落とし穴（実際に誤診した）**:
  - **「スタートメニューから起動すれば直る」は誤り**。スタートメニュー・タスクバー・デスクトップは
    すべて Explorer の子なので、同じ壊れた環境を継承する。
  - **「サインアウト→サインイン／再起動で直る」も誤り**。上限を超えたままなら、ログオンのたびに
    何か（環境変更をブロードキャストするもの）が走った時点で再発する。
  - `Start-Process explorer.exe "<exe パス>"` は**新しい explorer が環境を作り直す**ため通ってしまい、
    誤った結論を出しやすい。**判定は必ずプロセスの PATH を実測する**。
  - PowerShell の関数内で進捗を文字列出力すると**戻り値に混ざる**。`if (Test-Xxx ...)` が
    2 要素配列＝常に真になり、二分探索が全部「成功」になった（実際にやらかした）。
    ログは `Write-Host`、戻り値は `[bool]` にキャストする。
- **トリガーは未確定**: 環境が差し替わった 5 秒の窓に残る唯一の記録が winget COM ログ
  （`IsCOMCall:1; Caller: explorer.exe`）。相関はあるが**因果は立証できていない**
  （そのログ自体はインベントリ参照のみでインストールも PATH 更新もしていない）。
- **予防**: 開発ツールを入れて PATH が伸びたら合計長を測る癖をつける。
  起動時プリフライト警告バー（v0.1.2〜）があるので、この状態は Studio 起動直後に画面で分かる。
- **診断手順（実行中プロセスの PATH を読む）**: PEB 経由で他プロセスの環境ブロックを読む。
  ...
  **全プロセスを走査して PATH 長でグループ化する**と、健全な系統（サービス/DCOM 由来）と壊れた系統
  （Explorer 由来）が一目で分離でき、時刻境界もそこから読める。
- **健全な PATH の基準値を得る方法**: `userenv.dll` の `CreateEnvironmentBlock(token, bInherit=FALSE)` を
  自分で呼ぶと、**ログオン時と同じ手順で組まれた正しい環境**が得られる。これと壊れたプロセスの
  PATH を比べれば「レジストリが悪いのか、生成側が悪いのか」を一発で切り分けられる。
